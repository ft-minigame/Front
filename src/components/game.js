...
const Game = () => {
    ...
    const start = { x: (window.innerWidth) /2 +100 , y: (window.innerHeight) / 2 + 50};
        ...
        function handleKeyDown(event) {
            ...
            switch (key) {
                case 'ArrowUp':
                    ...
                    setPosition(pos => ({ ...pos, y: Math.max(pos.y - 10, -pos.y + containerHeight / 2, 0) }));
                    setImage(back);
                    break;
                case 'ArrowDown':
                    setPrevPosition(position);
                    setPosition(pos => ({ ...pos, y: Math.min(pos.y + 10, containerHeight / 2 - 100 + pos.y, containerHeight - 100) }));
                    setImage(front);
                    break;
                case 'ArrowLeft':
                    setPrevPosition(position);
                    setPosition(pos => ({ ...pos, x: Math.max(pos.x - 10, -pos.x + containerWidth / 2, 0) }));
                    setImage(left);
                    break;
                case 'ArrowRight':
                    setPrevPosition(position);
                    setPosition(pos => ({ ...pos, x: Math.min(pos.x + 10, containerWidth / 2 - 100 + pos.x, containerWidth - 100) }));
                    setImage(right);
                    break;
                default:
                    break;
